...
def main_worker(gpu, ngpus_per_node, args):
    ...
    writer = SummaryWriter('./log_moco_imgnet32_3')
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, tb_writer=writer)

        if not args.multiprocessing_distributed or (
            args.multiprocessing_distributed and args.rank % ngpus_per_node == 0
        ):
            if (epoch % 5 == 0) or ((args.epochs - epoch)<6) or (epoch % 3 == 0):
                save_checkpoint(
                    {
                        "epoch": epoch + 1,
                        "arch": args.arch,
                    ...
                    is_best=False,
                    filename="./imagenet32_ckpt/checkpoint_{:04d}.pth.tar".format(epoch),
                )
    writer.close()
...
def train(train_loader, model, criterion, optimizer, epoch, args, tb_writer):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0 and (epoch+i) != 0:
            progress.display(i)
            tb_writer.add_scalar('loss', loss.item(), step)
            tb_writer.add_scalar('acc1', acc1[0], step)
            tb_writer.add_scalar('acc5', acc5[0], step)
