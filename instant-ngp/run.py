		...
		with tqdm(desc="Training", total=n_steps, unit="steps") as t:
			...
			while testbed.frame():
				...
				if now - tqdm_last_update > 0.1:
					...
					training_loss_list.append((iii, testbed.loss))

	np.savetxt("loss.txt", training_loss_list)
	...
	if args.test_transforms:
		...
		with tqdm(range(testbed.nerf.training.dataset.n_images), unit="images", desc=f"Rendering test frame") as t:
			for i in t:
				...
				if i <= 100:
					write_image(f"results/icedundun/ref{i}.png", ref_image)
					write_image(f"results/icedundun/out{i}.png", image)

					# diffimg = np.absolute(image - ref_image)
					# diffimg[...,3:4] = 1.0
					# write_image(f"results/diff{i}.png", diffimg)
